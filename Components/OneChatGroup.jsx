...
function OneChatGroup(props) {
  ...
  useEffect(() => {
    const typingRef = ref_la_discussion.child("typing");
    typingRef.on("value", (snapshot) => {
      const typingUsers = [];
      snapshot.forEach((child) => {
        // console.log("Child: ", child.key, child.val());

        if (child.key !== props.currentId && child.val()) {
          typingUsers.push(child.key);
          //   console.log("Typing users: ", child.val());
        }
      });
      console.log("Typing users: ", typingUsers);
      setOtherTyping(typingUsers.length > 0);
    });
    return () => typingRef.off();
  }, []);
  console.log("otherTyping: ", otherTyping);
